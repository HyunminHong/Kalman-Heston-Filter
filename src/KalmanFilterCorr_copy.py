...
class HestonKalmanFilterCorr:
    ...
    def _get_measurement_matrices(self, param_dict: Dict[str, float]) -> tuple:
        ...
        if self.measurement_type == MeasurementType.RETURNS:
            ...
            sigma_vec = np.array([[np.sqrt(self.dt), 0], [0, param_dict['sigma'] * np.sqrt(self.dt)]])
        ...
        for t in range(T):
            ...
            if self.measurement_type == MeasurementType.BOTH:
                if t < 2:
                    ...
                    S_inv = np.linalg.pinv(sigma_vec) if np.linalg.cond(sigma_vec) > 1e10 else np.linalg.inv(sigma_vec)
                    K_1 = xi * np.sqrt(V_filt[t-1] * self.dt)
                    K_2 = np.sqrt(V_filt[t-2] * self.dt)
                    K_3 = (rho_vec.T @ S_inv @ innov_pred) # in R(1x1)

                    # correlation‐driven correction
                    V_corr_pred = (K_1 / K_2) * K_3.item()
                    P_corr_pred = 1.0 - (rho_vec.T @ rho_vec).item() + ((K_3.item() / K_2) ** 2)
                ...
                if self.measurement_type == MeasurementType.RETURNS:
                    if t < 2:
                        ...
                        # Kalman prediction gain
                        S_inv = 0.0 if sigma_val < 1e-12 else 1 / sigma_val
                        K_1 = xi * np.sqrt(V_filt[t-1] * self.dt)
                        K_2 = np.sqrt(V_filt[t-2] * self.dt)
                        K_3 = (rho_val * S_inv * innov_pred) # in R(1x1)

                        # correlation‐driven correction
                        V_corr_pred = (K_1 / K_2) * K_3
                        P_corr_pred = 1.0 - (rho_val ** 2) + ((K_3 / K_2) ** 2)
