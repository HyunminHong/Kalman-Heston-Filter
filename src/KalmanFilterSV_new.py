...
class HestonKalmanFilter:
    ...
    def _get_measurement_matrices(self, param_dict: Dict[str, float]) -> tuple:
        ...
        return mu_vec, beta_vec, sigma_vec
    ...
    def filter(self, params: np.ndarray, returns: Optional[np.ndarray] = None, rv: Optional[np.ndarray] = None) -> Dict[str, np.ndarray]:
        ...
        mu_vec, beta_vec, sigma_vec = self._get_measurement_matrices(param_dict)
        ...
        for t in range(T):
            ### Prediction Step (same for all measurement types) 
            # State equation: V_t+1 = V_t + kappa*(theta - V_t)*dt + xi*sqrt(V_t)*sqrt(dt)*eta_t
            V_pred[t] = V_filt_prev + kappa * (theta - V_filt_prev) * self.dt
            
            # State noise variance Q, scaled with dt
            Q = xi**2 * V_filt_prev * self.dt
            
            # Propagate variance
            P_pred[t] = (1 - kappa * self.dt)**2 * P_filt_prev + Q
    ...
    def fit(self, returns: Optional[np.ndarray] = None, rv: Optional[np.ndarray] = None, 
        ...
        if initial_params is None:
            ...
            if self.measurement_type == MeasurementType.RETURNS:
                ...
                initial_params = np.array([kappa_init, theta_init, xi_init, mu_init])
            elif self.measurement_type == MeasurementType.RV:
                sigma_init = 0.5
