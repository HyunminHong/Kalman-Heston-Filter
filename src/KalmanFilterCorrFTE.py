        ...
        for t in range(T):
            ...
            if self.measurement_type == MeasurementType.BOTH:
                if t == 0:
                    ...
                    H = (beta_vec @ beta_vec.T) * P_pred_prev + (sigma_vec @ sigma_vec.T) * np.max(V_filt_prev, 0)
                    H_inv = np.linalg.pinv(H) if np.linalg.cond(H) > 1e10 else np.linalg.inv(H)
                    K_t = np.sqrt(np.max(V_filt_prev, 0)) * (sigma_rho.T @ H_inv) # in R(1x2)
                    ...
                    V_corr_pred = xi * np.sqrt(np.max(V_filt_prev, 0) * self.dt) * (K_t @ innov_pred).item()
                    P_corr_pred = 1.0 - np.sqrt(V_filt[t-1]) * (K_t @ sigma_rho)

                # base Euler
                V_pred[t] = V_filt_prev + kappa * (theta - np.max(V_filt_prev, 0)) * self.dt + V_corr_pred

                # State noise variance Q, scaled with dt
                Q = xi**2 * np.max(V_filt_prev, 0) * self.dt
                ...
                if self.measurement_type == MeasurementType.RETURNS:
                    if t == 0:
                        ...
                        H = (beta_val ** 2) * P_pred_prev + (sigma_val ** 2) * np.max(V_filt_prev, 0)
                        H_inv = 0.0 if H < 1e-12 else 1 / H

                        K_t = np.sqrt(np.max(V_filt_prev, 0)) * (sigma_rho.item() * H_inv) # in R(1x1)

                        # previous innovation
                        innov_pred = y[t-1] - mu_val - beta_val * V_pred[t-1] # in R(1x1)

                        # correlation‐driven correction
                        V_corr_pred = xi * np.sqrt(np.max(V_filt_prev, 0) * self.dt) * (K_t * innov_pred)
